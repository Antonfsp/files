...
for agent in agents:
    ...
    if model.solve():
        ...
        agent.profit_first_stage_full_cooperation = - model.edges_costs.solution_value
    else:
...
for agent in agents:
    agents_minimal_profit.append(agent.profit_first_stage_partial_cooperation - agent.profit_first_stage_full_cooperation)
...
if model.solve():
     ...
     full_cooperation_planner.satisfied_demands = cpmdl.recover_data_full_cooperation(model, full_cooperation_planner.edges, full_cooperation_planner.demands)
else:
    print("Problem has no solution")

# ------ Recover how much each agent earn in the second stage
for d in full_cooperation_planner.satisfied_demands:
    agents[d[2]].profit_second_stage_full_cooperation += agents[d[2]].demands[(d[0],d[1])].units*agents[d[2]].demands[(d[0],d[1])].revenue
    for e in full_cooperation_planner.satisfied_demands[d]:
        if e[2] != d[2]:
            ...
            agents[e[2]].profit_second_stage_full_cooperation += price
            agents[d[2]].profit_second_stage_full_cooperation -= price
...
for agent in agents:
    print('Agent %s has earned %s in the first stage and %s in the second stage' % (agent.id,agent.profit_first_stage_full_cooperation,agent.profit_second_stage_full_cooperation))
    print('what makes a total of %s' % agent.total_profit_full_cooperation)